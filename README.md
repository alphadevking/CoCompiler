...
*   **Enhanced Compiler Phase Logging:** Comprehensive logging has been added for each compiler phase (Lexical Analysis, Syntax Analysis, Semantic Analysis, TAC Generation, CFG Generation, SSA Generation, Intermediate Code Generation, Code Optimization, Code Generation, and Execution), providing detailed insights into the compilation process.
*   **Improved String Literal Handling in VM:** Fixed an issue where string literals were not correctly printed by the Virtual Machine.
*   **Support for Binary Expressions in AST:** The Abstract Syntax Tree now includes a `BINARY_EXPRESSION` node type, allowing for better representation and handling of binary operations.
