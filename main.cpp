...
void print_ssa_tac_minimal() {
    std::cout << "SSA generated (with phi functions where needed)" << std::endl;
...
void run_compiler(const std::string& source_code) {
    ...
    if (ast) {
        ...
        std::cout << "AST generation failed." << std::endl;
        return;
    }
    ...
    if (optimized_ast != ast) {
        ...
        // Use a fresh Compiler for the optimized AST
        Compiler opt_compiler;
        bytecode = opt_compiler.compile(optimized_ast);
        const auto& sem_errors = opt_compiler.getSemanticErrors();
        if (!sem_errors.empty()) {
        ...
        // Use the string literals from the optimized compiler for execution
        std::cout << "\n=== PHASE 7: CODE GENERATION ===" << std::endl;
        std::cout << "Code generation complete." << std::endl;
        std::cout << "\n=== PHASE 8: EXECUTION ===" << std::endl;
        VM vm;
        double result = vm.run(bytecode, opt_compiler.getStringLiterals());
        std::cout << "Execution complete. Result: " << result << std::endl;
    } else {
        ...
        std::cout << "\n=== PHASE 8: EXECUTION ===" << std::endl;
        VM vm;
        double result = vm.run(bytecode, compiler.getStringLiterals());
        std::cout << "Execution complete. Result: " << result << std::endl;
    }
